...
function loadshow(){
    var loadStr='<ul id="loading" style="opacity: 0;display: table;width: 100%;height:100%;position: absolute;top:0;left:0;z-index: 30;"> ' +
        ...
        '<img style="width: 43px;background: black;padding: 15px;border-radius: 6px;" src="../Public/js/loadGif/load.gif"> ' +
        '</li> ' +
        '</ul>';
    $("body").append(loadStr);
    $("#loading").animate({
...
/*点击背景退出模态框*/
var issacModel =false;
$(".dialog-issac").click(function(){
    if(!issacModel){
        var target =$(this).parent().attr("id");
        modelOutT("#"+target,"-30px");
    }else{
        window.issacModel =false;
    }
});
$(".blockIssac").click(function(){
    window.issacModel =true;
});